...
var pager = {
    ...
    show_msg: function(content){
        ...
        try{
            ...
            for(var sub in matched){
                content = content.split('['+sub+']').join('<img src="'+'http://static.live.xiguapi.tv/'+faces.map[sub]+'" />');
            }
        }catch(e){
            console.log(e);
        }
...
$(document).ready(function () {
    ...
    $("#chat_operate_area .icon-emjo").click(function(){
        var _pos = $("#chat_operate_area .icon-emjo").position();
    ...
    //表情
    var face_init = false;
    var face_render = function(data){
        if(data==null || face_init)return;
        var html = "",k = 0;
        var link_html = $('<div class="tab">');
        for(var i in data) {
            var _each = data[i];
            if(k==0)
            {
            ...
            if (_each.faces != null && _each.faces.length > 0) {
                for (var j=0;j<_each.faces.length;j++) {
                    html += '<a class="can-choose" data-id="'+_each.faces[j].txt+'" data-src="'+'http://static.live.xiguapi.tv/'+_each.faces[j].url+'"><img title="'+_each.faces[j].title+'" alt="'+_each.faces[j].title+'" src="'+'http://static.live.xiguapi.tv/'+_each.faces[j].url+'"></a>'
                }
            }
            html += '</div>';
            k++;
