    ...
    show_msg: function(content,user,board_str){
        ...
        try{
            ...
            for(let sub in matched){
                content = content.split('['+sub+']').join('<img src="'+'/images/'+faces.map[sub]+'" />');
            }
        }catch(e){
            console.log(e);
        }
        ...
        for(let i in data) {
            ...
            if (_each.faces != null && _each.faces.length > 0) {
                for (let j=0;j<_each.faces.length;j++) {
                    html += '<a class="can-choose" data-id="'+_each.faces[j].txt+'" data-src="'+'/images/'+_each.faces[j].url+'"><img title="'+_each.faces[j].title+'" alt="'+_each.faces[j].title+'" src="'+'/images/'+_each.faces[j].url+'"></a>'
                }
            }
            html += '</div>';
            k++;
