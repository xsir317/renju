<?php
/**
 * 五子棋（禁手）判断胜负的类，棋盘用位操作来实现。
 * 这要是用C来写得多快。。。
 * Date: 2019/1/20
 * 参考 http://php.net/manual/zh/language.operators.bitwise.php
 */
...
class RenjuBoardTool_bit
{
    ...
    private function setStone($stone = '.',$coordinate = [])
    {
        ...
        $row = $this->board[$coordinate[0] -1];
        //给黑、白的所在位置先一律置空
        $row &= ~(1 << ($coordinate[1] -1));
        $row &= ~(1 << ($coordinate[1] -1 + 16));
        switch ($stone)
        {
            case self::EMPTY_STONE:
                break;
            case self::WHITE_STONE:
                $row |= (1 << ($coordinate[1] -1 + 16));
                break;
            case self::BLACK_STONE:
                $row |= (1 << ($coordinate[1] -1));
                break;
        }
    ...
    private function moveDirection($direction)
    {
        ...
        if($next[0] < 1 || $next[0] > 15 || $next[1] < 1 || $next[1] > 15 )
        {
    ...
    private function _($coordinate = [])
    {
        ...
        if($this->board[$coordinate[0] - 1] & (1 << $coordinate[1] - 1))
        {
            return self::BLACK_STONE;
        }
        if($this->board[$coordinate[0] - 1] & (1 << $coordinate[1] - 1 + 16))
        {
